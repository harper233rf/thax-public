...
import static com.matt.forgehax.Globals.LOGGER;
...
public final class AuthHelper extends YggdrasilUserAuthentication {
  ...
  public void loadCurrentSession() {
    ...
    credentials.put("username", MC.getSession().getUsername());
    credentials.put("userid", MC.getSession().getPlayerID());
    credentials.put("accessToken", MC.getSession().getToken());
    loadFromStorage(credentials); // logs out
  }
  ...
  public boolean isSessionValid() {
    try {
      ...
      // why wtf, there's a try/catch inside!
      return false;
    }
  }
  ...
  public static String encrypt(String encryptString, String iv, String key, String keySalt) {
    try {
      ...
      e.printStackTrace();
      LOGGER.error("Encryption error: " + e.toString());
    }
  ...
  public static String decrypt(String decryptString, String iv, String key, String keySalt) {
    try {
      ...
      e.printStackTrace();
      LOGGER.error("Decryption error: " + e.toString());
    }
  ...
  public static String getSecretKey() throws IOException {
    if (AccountManager.INSTANCE.mode.get() == AccountManager.SecretKeyOptions.DEFAULT) {
      ...
      LOGGER.error("getSecretKey() returned null.");
      return null;
