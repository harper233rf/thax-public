...
public class AccountManager extends ServiceMod {
  ...
  public final Setting<Integer> mpwDelay =
    getCommandStub()
      ...
      .name("delay")
      .description("Delay in seconds after which masterPassword is reset, \"0\" to disable.")
      .defaultTo(120) // 2 min
      .min(0)
      .max(900)
      .build();

  public final Setting<Boolean> auto_reauth =
    getCommandStub()
      .builders()
      .<Boolean>newSettingBuilder()
      .name("auto-reauth")
      .description("Check if your session is valid before connecting and refresh if needed")
      .defaultTo(true)
      .build();
  ...
  public final Setting<Integer> delay =
    getCommandStub()
      ...
      .<Integer>newSettingBuilder()
      .name("delay-reauth")
      .description("Delay in seconds between auto reauth attempts")
      .min(0)
      .max(300)
      .defaultTo(60)
      .build();
  ...
  public AccountManager() {
    super("AccountManager", "Change active user session, alt manager");
    INSTANCE = this;
  }
  ...
  protected void onLoad() {
    super.onLoad();
    auth.loadCurrentSession(); // so we can refresh the original session too, but it doesn't work
    timer.start();
    ...
    getCommandStub()
      ...
      .newCommandBuilder()
      .name("reauth")
      .description("Refresh current login token")
      .processor(data -> {
        try {
          auth.refresh();
          Helper.printInform("Correctly refreshed session");
    ...
    getCommandStub()
      ...
      .newCommandBuilder()
      .name("login")
      .description("Login with provided alias")
      .processor(data -> {
        data.requiredArguments(1);
    ...
    getCommandStub()
      ...
      .newCommandBuilder()
      .name("save")
      .description("Save or edit an account, <alias> <email> <password>")
      .processor(
        data -> {
          ...
          if (!account.exists()) {
            ...
            try {
              ...
              LOGGER.info("Credentials decrypted (for credentials comparison).");
              ...
              if (credentials != null) {
                ...
                if (email.equalsIgnoreCase(emailIn) && password.equals(passwordIn)) {
                  ...
                  if (email.equalsIgnoreCase(emailIn)) {
                    LOGGER.info("Email for \"" + alias + "\" didn't change.");
                  } else if (password.equals(passwordIn)) {
                    LOGGER.info("Password for \"" + alias + "\" didn't change.");
                  }
                  ...
                  LOGGER.info("Credentials encrypted (account was edited).");
    ...
    getCommandStub()
      ...
      .processor(
        data -> {
          ...
          auth.loadCurrentSession(); // load original session token into the authenticator
        })
      .build();
  ...
  @SubscribeEvent
  public void onConnect(GuiOpenEvent event) {
    if (event.getGui() instanceof GuiConnecting) {
      if (!auto_reauth.get() || !timer.hasTimeElapsed(delay.get() * 1000)) return;
      if (auth.getAuthenticatedToken().equals("<ACCESS TOKEN>")) {
        Helper.printWarning("Access token is obfuscated, cannot verify or refresh");
        return;
      }
      if (!auth.isSessionValid()) {
        try {
          timer.start();
          auth.refresh();
          Helper.printInform("Refreshed expired session");
        } catch (AuthenticationException e) {
          Helper.printError(e.getMessage());
        }
      }
  ...
  public boolean login(String alias) {
    ...
    if (account.exists()) {
      try {
        ...
        LOGGER.info("Credentials decrypted (for login or reauth).");
  ...
  public void checkTimer() {
    if (mpwDelay.get() != 0 && mpwTimer.hasTimeElapsed(mpwDelay.get() * 1000)) {
      masterPassword = null;
    }
  }
  ...
  public static void generateSecretKey() {
    ...
    LOGGER.info("Saved " + authFile.getName() + " file.");
  }
  ...
  public static void saveEncryptCheck(char[] masterPw) throws IOException {
    ...
    LOGGER.info("Saved " + checkFile.getName() + " file.");
  }
